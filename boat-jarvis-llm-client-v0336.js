...
// Calls same-origin backend; executes only allow-listed browser tools. API keys stay server-side.
(()=>{
'use strict';
...
async function chat(text,context={}){const q=String(text||'').normalize('NFKC').trim();if(!q)return'';const conversation=context?.conversation||context||{};const history=Array.isArray(conversation?.history)?conversation.history:[];const base={message:q,state:{...context,jarvisTools:capabilities()},history};const prev=previous();if(prev)base.previous_response_id=prev;
  ...
  if(calls.length){const outputs=[];for(const call of calls){let result;try{result=await executeTool(call)}catch(e){result={ok:false,error:String(e?.message||e)}}outputs.push({call_id:call.call_id,name:call.name,output:result});}
    data=await post({message:q,state:{...context,jarvisTools:capabilities()},history:[],previous_response_id:data.response_id||previous(),tool_outputs:outputs});if(data.response_id)remember(data.response_id);
  }
...
window.BOAT_JARVIS_LLM_CLIENT_V0336=Object.freeze({version:VERSION,chat,executeTool,clearConversationState:clear,previousResponseId:previous,capabilities,endpoint:ENDPOINT,clientApiKey:false,responseContinuity:true,toolAware:true,toolExecution:true});
})();