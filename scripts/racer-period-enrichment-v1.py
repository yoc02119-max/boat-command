...
def build(archive_root:Path,out_root:Path):
    ...
    for slug,x in iter_rich():
        ...
        venues.append({
          ...
          "classMismatchRows":matched-class_match,
          "classMatchCoverageOfMatched":round(class_match/matched,6) if matched else None,
          "safeJoinCoverage":round(class_match/boat_rows,6) if boat_rows else None,
    ...
    audit={
      ...
      "recommendedJoinPolicy":"RACE_DATE_PERIOD + REGISTRATION + CLASS_MATCH_REQUIRED",
      "periods":period_meta,
      "totals":{
        "venues":len(venues),"boatRows":total,"matchedBoatRows":matched,
        "matchCoverage":round(matched/total,6) if total else None,
        "classMatchRows":global_counts["classMatch"],
        "classMismatchRows":matched-global_counts["classMatch"],
        "classMatchCoverageOfMatched":round(global_counts["classMatch"]/matched,6) if matched else None,
        "safeJoinCoverage":round(global_counts["classMatch"]/total,6) if total else None,
