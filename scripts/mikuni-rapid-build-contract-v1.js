...
const config=require('../venues/mikuni/config-v1.json');

assert.equal(model.version,'MIKUNI-RESEARCH-MODEL-V1');
assert.equal(model.resultInput,false);
assert.equal(model.payoutInput,false);
assert.equal(config.venueCode,'10');
...
const k=registry.resolve('10');
assert.equal(k.slug,'mikuni');
assert.equal(k.state,'LIVE_SIMULATION');
assert.equal(k.model.global,'BOAT_COMMAND_MIKUNI_RESEARCH_MODEL_V1');
assert.equal(k.capabilities.try,true);
assert.equal(k.capabilities.realMoney,false);

const history=fs.readFileSync('.github/workflows/mikuni-history-bootstrap-v1.yml','utf8');
const program=fs.readFileSync('.github/workflows/mikuni-program-snapshot-v1.yml','utf8');
const shadow=fs.readFileSync('.github/workflows/mikuni-shadow-research-v1.yml','utf8');
const result=fs.readFileSync('.github/workflows/mikuni-result-collector-v1.yml','utf8');
assert.match(history,/--venue 10/);
assert.match(program,/jcd=10/);
assert.match(result,/jcd=10/);
assert.match(shadow,/live\/mikuni/);
...
console.log('MIKUNI_RAPID_BUILD_CONTRACT_PASS');
