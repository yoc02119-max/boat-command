#!/usr/bin/env node
'use strict';
const fs=require('fs'),model=require('../karatsu-research-model-v1.js');
const input=process.argv[2]||'karatsu-history-bootstrap-v1.json',output=process.argv[3]||'karatsu-baseline-backtest-v1.json';
const db=JSON.parse(fs.readFileSync(input,'utf8'));if(db.venueCode!=='23')throw new Error('KARATSU_ONLY');
...
function programFrom(t){return{venue:'KARATSU',venueCode:'23',date:t.d,race:Number(t.r),raceType:String(t.t||''),boats:t.c.map((c,i)=>({lane:i+1,class:c})),resultEndpointsIncluded:false,resultIncluded:false,exhibitionIncluded:false}}
...
if(!calibration.length||!calibration[0].m4.races)throw new Error('KARATSU_CALIBRATION_EMPTY');
...
const report={
  schema:'boat-command-karatsu-baseline-backtest-v1',version:'KARATSU-BASELINE-BACKTEST-V1',venue:'KARATSU',venueCode:'23',source:input,
  ...
  baselineArchitecture:'KARATSU_CLASS_NEIGHBOR_BASELINE',candidateArchitecture:'KARATSU_EMPIRICAL_FIRST_SECOND_TRANSITION_PLUS_CLASS_NEIGHBORS',
  ...
  promotionBlockers:['KARATSU_FORWARD_36_RACES_NOT_READY','KARATSU_FORWARD_60_RACES_NOT_READY','KARATSU_FORWARD_MODEL_UPLIFT_NOT_READY']
};
