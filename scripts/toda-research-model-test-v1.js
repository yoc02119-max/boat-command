...
const classes=['A1','A2','B1','B2'],history=[];
for(let i=0;i<360;i++){
  const day=1+Math.floor(i/12),m=1+Math.floor((day-1)/28),d=1+((day-1)%28);
  const date=`2026-${String(m).padStart(2,'0')}-${String(d).padStart(2,'0')}`,r=i%12+1,a=i%6+1,b=a%6+1,c=b%6+1;
...
for(const mode of ['CLASS_BASELINE','PROGRAM_ONLY']){
  const d=model.distribution(program,history,{targetDate:'2026-12-31',config:{recencyWindow:300},mode});
  assert.equal(d.version,'TODA-RESEARCH-MODEL-V1');assert.equal(d.mode,mode);assert.equal(d.historyRows,300);assert.ok(Math.abs(d.sum-1)<1e-9);
  const p=model.select(d,{count:4});assert.equal(p.length,4);assert.equal(new Set(p.map(x=>x.order)).size,4);
}
...
assert.throws(()=>model.distribution(program,history,{targetDate:'2026-12-31',mode:'FULL_PRE'}),/MODE_INVALID/);
