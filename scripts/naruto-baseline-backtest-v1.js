#!/usr/bin/env node
'use strict';
const fs=require('fs'),model=require('../naruto-research-model-v1.js');
const input=process.argv[2]||'naruto-history-bootstrap-v1.json',output=process.argv[3]||'naruto-baseline-backtest-v1.json';
const db=JSON.parse(fs.readFileSync(input,'utf8'));if(db.venueCode!=='14')throw new Error('NARUTO_ONLY');
...
function programFrom(t){return{venue:'NARUTO',venueCode:'14',date:t.d,race:Number(t.r),raceType:String(t.t||''),boats:t.c.map((c,i)=>({lane:i+1,class:c})),resultEndpointsIncluded:false,resultIncluded:false,exhibitionIncluded:false}}
...
if(!calibration.length||!calibration[0].m4.races)throw new Error('NARUTO_CALIBRATION_EMPTY');
...
const report={
  schema:'boat-command-naruto-baseline-backtest-v1',version:'NARUTO-BASELINE-BACKTEST-V1',venue:'NARUTO',venueCode:'14',source:input,
  ...
  baselineArchitecture:'NARUTO_CLASS_NEIGHBOR_BASELINE',candidateArchitecture:'NARUTO_EMPIRICAL_FIRST_SECOND_TRANSITION_PLUS_CLASS_NEIGHBORS',
  ...
  promotionBlockers:['NARUTO_FORWARD_36_RACES_NOT_READY','NARUTO_FORWARD_60_RACES_NOT_READY','NARUTO_FORWARD_MODEL_UPLIFT_NOT_READY']
};
