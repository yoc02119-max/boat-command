...
const input=process.argv[2]||'ashiya-history-bootstrap-v1.json';
const output=process.argv[3]||'ashiya-history-analysis-v1.json';
const db=JSON.parse(fs.readFileSync(input,'utf8'));
if(db.venueCode!=='21')throw new Error('ASHIYA_HISTORY_TARGET_INVALID');
const rows=[...(db.races||[])].sort((a,b)=>String(a.d).localeCompare(String(b.d))||Number(a.r)-Number(b.r));
if(rows.length<300)throw new Error('ASHIYA_HISTORY_MIN_300_REQUIRED');
...
const report={
  schema:'boat-command-ashiya-history-analysis-v1',
  version:'ASHIYA-HISTORY-ANALYSIS-V1',
  venue:'ASHIYA',venueCode:'21',
  ...
  modelPolicy:{
    ...
    nextStage:'STRICT_ASHIYA_WALK_FORWARD_BASELINE'
  }
};
