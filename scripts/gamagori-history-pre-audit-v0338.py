...
"""
import argparse, json, re, urllib.request, zipfile, io, os, subprocess, tempfile
...
def main():
    ...
    yy=a.date[2:4]; mm=a.date[4:6]; dd=a.date[6:8]
    url=f'https://www1.mbrace.or.jp/od2/B/{a.date[:4]}/{mm}/b{yy}{mm}{dd}.lzh'
    data=download(url); text=decode(unpack_lzh(data))
    lines=text.splitlines()
    # Audit only Gamagori-looking sections and representative entry lines; no result source is opened.
    ...
    report={
      ...
      'classEntryLines':len(class_lines),
      'sampleLines':sample,
