...
for(let race=1;race<=12;race++){
  ...
  const out=path.join(root,'live','toda',date,'shadow','program-only',`race-${race}.json`);if(fs.existsSync(out))continue;
  const dist=model.distribution(program,db.races,{targetDate:date,config:baseline.calibration.selected}),picks=model.select(dist,{count:4});
  const payload={schema:'boat-command-toda-shadow-research-v1',version:'TODA-SHADOW-RESEARCH-V1',venue:'TODA',venueCode:'02',date,race,mode:'PROGRAM_ONLY',generatedAt:now.iso,deadline:program.deadline||null,modelVersion:model.version,modelArchitecture:dist.architecture,modelConfig:dist.config,historyRows:dist.historyRows,historyCutoff:db.cutoff||null,sources:{programPath:path.relative(root,pp).replace(/\\/g,'/'),programSha256:hash(pp),programFetchedAt:program.fetchedAt||null},picks:picks.map(x=>x.order),probabilities:picks.map(x=>({order:x.order,probability:x.probability})),probabilitySum:dist.sum,resultInput:false,payoutInput:false,researchOnly:true,productionEnabled:false,tryEnabled:false,cashNeutral:true,immutableAfterFirstWrite:true};
