...
const output=process.argv[3]||'mikuni-history-analysis-v1.json';
const db=JSON.parse(fs.readFileSync(input,'utf8'));
if(db.venueCode!=='10')throw new Error('MIKUNI_HISTORY_TARGET_INVALID');
const rows=[...(db.races||[])].sort((a,b)=>String(a.d).localeCompare(String(b.d))||Number(a.r)-Number(b.r));
if(rows.length<300)throw new Error('MIKUNI_HISTORY_MIN_300_REQUIRED');
...
const report={
  schema:'boat-command-mikuni-history-analysis-v1',
  version:'MIKUNI-HISTORY-ANALYSIS-V1',
  ...
  modelPolicy:{
    ...
    nextStage:'STRICT_MIKUNI_WALK_FORWARD_BASELINE'
  }
};
