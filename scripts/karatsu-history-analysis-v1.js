...
const input=process.argv[2]||'karatsu-history-bootstrap-v1.json';
const output=process.argv[3]||'karatsu-history-analysis-v1.json';
const db=JSON.parse(fs.readFileSync(input,'utf8'));
if(db.venueCode!=='23')throw new Error('KARATSU_HISTORY_TARGET_INVALID');
const rows=[...(db.races||[])].sort((a,b)=>String(a.d).localeCompare(String(b.d))||Number(a.r)-Number(b.r));
if(rows.length<300)throw new Error('KARATSU_HISTORY_MIN_300_REQUIRED');
...
const report={
  schema:'boat-command-karatsu-history-analysis-v1',
  version:'KARATSU-HISTORY-ANALYSIS-V1',
  venue:'KARATSU',venueCode:'23',
  ...
  modelPolicy:{
    ...
    nextStage:'STRICT_KARATSU_WALK_FORWARD_BASELINE'
  }
};
