...
const input=process.argv[2]||'tokuyama-history-bootstrap-v1.json';
const output=process.argv[3]||'tokuyama-history-analysis-v1.json';
const db=JSON.parse(fs.readFileSync(input,'utf8'));
if(db.venueCode!=='18')throw new Error('TOKUYAMA_HISTORY_TARGET_INVALID');
const rows=[...(db.races||[])].sort((a,b)=>String(a.d).localeCompare(String(b.d))||Number(a.r)-Number(b.r));
if(rows.length<300)throw new Error('TOKUYAMA_HISTORY_MIN_300_REQUIRED');
...
const report={
  schema:'boat-command-tokuyama-history-analysis-v1',
  version:'TOKUYAMA-HISTORY-ANALYSIS-V1',
  venue:'TOKUYAMA',venueCode:'18',
  ...
  modelPolicy:{
    ...
    nextStage:'STRICT_TOKUYAMA_WALK_FORWARD_BASELINE'
  }
};
