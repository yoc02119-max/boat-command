...
const today=jstDate();
const requestedDate=process.argv[2]||today;

function deadlineEpoch(date,hm){
  const m=String(hm||'').match(/^(\d{1,2}):(\d{2})$/);if(!m)return NaN;
  const utc=Date.parse(`${date}T${String(m[1]).padStart(2,'0')}:${m[2]}:00+09:00`);
...
function safePrediction(adapter,date,race){
  ...
  if(x.productionEnabled!==false&&x.researchOnly===true)return null;
  ...
  const ddl=deadlineEpoch(date,x.deadline);
  ...
  return {
    ...
    predictionPath:path.relative(ROOT,p).replace(/\\/g,'/'),
    predictionSha256:sha256(raw),
    modelVersion:x.modelVersion||null,
    picks,
    pickCoverage:pickProbs.reduce((s,v)=>s+v,0),
    topProbability:Math.max(...pickProbs),
    resultInput:false,payoutInput:false,immutableAfterFirstWrite:true
  };
}
function priorPortfolio(){
  return read(OUTPUT_PATH)||{
    schema:'boat-command-shared-try-portfolio-v1',
    version:'SHARED-TRY-PORTFOLIO-V1',
    startingBankrollYen:Number(config.startingBankrollYen)||1000000,
    bankrollYen:Number(config.startingBankrollYen)||1000000
  };
}
...
function buildSelection(date,available){
  ...
  const now=Date.now(),safety=(Number(config.deadlineSafetyMinutes)||5)*60000;
  const candidates=[];
  for(const a of config.activeAdapters||[]){
    const venue=[];
    for(let race=1;race<=12;race++){
      const x=safePrediction(a,date,race);if(!x)continue;
  ...
  candidates.sort((a,b)=>b.relativeConfidence-a.relativeConfidence||b.pickCoverage-a.pickCoverage||a.venueCode.localeCompare(b.venueCode)||a.race-b.race);
  ...
  const stakePerPick=Number(config.stakePerPickYen)||100,picksPerTry=Number(config.picksPerTry)||4;
  const stakePerRace=stakePerPick*picksPerTry;
  ...
  const selected=candidates.slice(0,Math.min(desired,affordable)).map((x,i)=>({
    ...
    deadline:x.deadline,generatedAt:x.generatedAt,modelVersion:x.modelVersion,
    predictionPath:x.predictionPath,predictionSha256:x.predictionSha256,
    picks:x.picks,pickCoverage:x.pickCoverage,venueMedianCoverage:x.venueMedianCoverage,
    relativeConfidence:x.relativeConfidence,stakePerPickYen:stakePerPick,stakeYen:stakePerRace,
  ...
  const payload={
    ...
    startingBankrollYen:Number(config.startingBankrollYen)||1000000,
    bankrollBeforeSelectionYen:available,
    policy:{
      selectionPolicy:config.selectionPolicy,selectionFraction:fraction,
      maxTryRacesPerDay:Number(config.maxTryRacesPerDay)||8,
      stakePerPickYen:stakePerPick,picksPerTry,deadlineSafetyMinutes:Number(config.deadlineSafetyMinutes)||5
    },
...
function rebuildPortfolio(){
  ...
  for(const x of rows){
    committed+=Number(x.stakeYen)||0;curveBalance-=Number(x.stakeYen)||0;
    peak=Math.max(peak,curveBalance);maxDrawdown=Math.max(maxDrawdown,peak-curveBalance);
    const r=resultFor(x),hit=!!(r&&x.picks.includes(r.trifecta)),ret=hit?Number(r.payout100||0)*(Number(x.stakePerPickYen||100)/100):0;
    if(r){
      settledStake+=Number(x.stakeYen)||0;returns+=ret;if(hit){hits++;lossStreak=0}else{lossStreak++;maxLossStreak=Math.max(maxLossStreak,lossStreak)}
      curveBalance+=ret;peak=Math.max(peak,curveBalance);maxDrawdown=Math.max(maxDrawdown,peak-curveBalance);
    }else pending+=Number(x.stakeYen)||0;
    const v=byVenue[x.venueCode]||(byVenue[x.venueCode]={venue:x.venue,tries:0,settled:0,hits:0,stakeYen:0,returnYen:0});
    v.tries++;v.stakeYen+=Number(x.stakeYen)||0;if(r){v.settled++;v.returnYen+=ret;if(hit)v.hits++}
    ledger.push({date:x.date,venueCode:x.venueCode,venue:x.venue,race:x.race,deadline:x.deadline,picks:x.picks,stakeYen:x.stakeYen,settled:!!r,result:r?.trifecta||null,payout100:r?.payout100??null,hit:r?hit:null,returnYen:r?ret:null});
  }
  for(const v of Object.values(byVenue)){
    v.profitYen=v.returnYen-v.stakeYen;
    v.roi=v.settled&&v.stakeYen?v.returnYen/v.stakeYen:null;
  ...
  const out={
    ...
    generatedAt:new Date().toISOString(),realMoney:false,fundingScope:'ALL_ACTIVE_VENUES_SHARED',
    ...
    returnYen:returns,profitYen:returns-settledStake,
    settledTries:ledger.filter(x=>x.settled).length,pendingTries:ledger.filter(x=>!x.settled).length,
    hits,hitRate:ledger.filter(x=>x.settled).length?hits/ledger.filter(x=>x.settled).length:null,
    roi:settledStake?returns/settledStake:null,maxDrawdownYen:maxDrawdown,maxConsecutiveLosses:maxLossStreak,
    byVenue,ledger,
...
const existing=allSelections();
if(existing.length)portfolio=rebuildPortfolio();
const available=Number(portfolio.bankrollYen??config.startingBankrollYen);
const selection=buildSelection(requestedDate,available);
