...
const input=process.argv[2]||'naruto-history-bootstrap-v1.json';
const output=process.argv[3]||'naruto-history-analysis-v1.json';
const db=JSON.parse(fs.readFileSync(input,'utf8'));
if(db.venueCode!=='14')throw new Error('NARUTO_HISTORY_TARGET_INVALID');
const rows=[...(db.races||[])].sort((a,b)=>String(a.d).localeCompare(String(b.d))||Number(a.r)-Number(b.r));
if(rows.length<300)throw new Error('NARUTO_HISTORY_MIN_300_REQUIRED');
...
const report={
  schema:'boat-command-naruto-history-analysis-v1',
  version:'NARUTO-HISTORY-ANALYSIS-V1',
  venue:'NARUTO',venueCode:'14',
  ...
  modelPolicy:{
    ...
    nextStage:'STRICT_NARUTO_WALK_FORWARD_BASELINE'
  }
};
