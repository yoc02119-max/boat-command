...
const date=process.argv[2]||new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Tokyo',year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date()),root=path.join(__dirname,'..','live','naruto',date);
function read(p){try{return JSON.parse(fs.readFileSync(p,'utf8'))}catch{return null}}
function lane(x,race,mode){if(!x||x.schema!=='boat-command-naruto-shadow-research-v1'||x.venueCode!=='14'||Number(x.race)!==race||x.mode!==mode||x.resultInput!==false||x.payoutInput!==false||x.immutableAfterFirstWrite!==true||!Array.isArray(x.picks)||x.picks.length!==4)return null;return x}
const rows=[];
for(let race=1;race<=12;race++){
  const result=read(path.join(root,'post',`race-${race}-result.json`));if(!result||result.venue!=='NARUTO'||result.venueCode!=='14'||result.preRaceDataIncluded!==false||result.resultEndpointsIncluded!==true)continue;
