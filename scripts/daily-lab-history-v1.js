...
for(let offset=29;offset>=0;offset--){
 ...
 const report=date===latest.date?latest:buildReport(date);
 // Only archived PRE-RACE captures are evaluated. Never generate missing predictions.
 if(!report.totals.capturedRaces&&date!==latest.date)continue;
 writeReport(report,path.join(dir,date+'.json'));
 dates.push({date,...report.totals});
...
writeReport({schema:'boat-command-daily-lab-history-v1',from:new Date(end-29*86400000).toISOString().slice(0,10),to:latest.date,
 researchOnly:true,automaticPromotion:false,dates,venues},path.join(dir,'index.json'));
