...
const lock=JSON.parse(fs.readFileSync('developer-lock-v1.json','utf8'));
...
assert.ok(portal.includes("developerCodeHash"));
assert.ok(portal.includes("boatCommand.developerSession.v1"));
...
assert.ok(auth.includes('PBKDF2-SHA256'));
assert.ok(auth.includes('JARVIS_GITHUB_EXECUTOR_TOKEN'));
assert.ok(auth.includes("developer-lock-v1.json"));
...
assert.equal(lock.schema,'boat-command-developer-lock-v1');
assert.equal(lock.algorithm,'PBKDF2-SHA256');
assert.ok(Number(lock.iterations)>=200000);
assert.equal(lock.configured,true);
assert.match(lock.hashHex,/^[0-9a-f]{64}$/);
