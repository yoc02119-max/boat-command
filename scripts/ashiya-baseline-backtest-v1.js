#!/usr/bin/env node
'use strict';
const fs=require('fs'),model=require('../ashiya-research-model-v1.js');
const input=process.argv[2]||'ashiya-history-bootstrap-v1.json',output=process.argv[3]||'ashiya-baseline-backtest-v1.json';
const db=JSON.parse(fs.readFileSync(input,'utf8'));if(db.venueCode!=='21')throw new Error('ASHIYA_ONLY');
...
function programFrom(t){return{venue:'ASHIYA',venueCode:'21',date:t.d,race:Number(t.r),raceType:String(t.t||''),boats:t.c.map((c,i)=>({lane:i+1,class:c})),resultEndpointsIncluded:false,resultIncluded:false,exhibitionIncluded:false}}
...
if(!calibration.length||!calibration[0].m4.races)throw new Error('ASHIYA_CALIBRATION_EMPTY');
...
const report={
  schema:'boat-command-ashiya-baseline-backtest-v1',version:'ASHIYA-BASELINE-BACKTEST-V1',venue:'ASHIYA',venueCode:'21',source:input,
  ...
  baselineArchitecture:'ASHIYA_CLASS_NEIGHBOR_BASELINE',candidateArchitecture:'ASHIYA_EMPIRICAL_FIRST_SECOND_TRANSITION_PLUS_CLASS_NEIGHBORS',
  ...
  promotionBlockers:['ASHIYA_FORWARD_36_RACES_NOT_READY','ASHIYA_FORWARD_60_RACES_NOT_READY','ASHIYA_FORWARD_MODEL_UPLIFT_NOT_READY']
};
