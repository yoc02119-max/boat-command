#!/usr/bin/env node
'use strict';
const fs=require('fs'),model=require('../toda-research-model-v1.js');
const input=process.argv[2]||'toda-history-bootstrap-v1.json',output=process.argv[3]||'toda-baseline-backtest-v1.json';
const db=JSON.parse(fs.readFileSync(input,'utf8'));if(db.venueCode!=='02')throw new Error('TODA_ONLY');
...
function programFrom(t){return{venue:'TODA',venueCode:'02',date:t.d,race:Number(t.r),raceType:String(t.t||''),boats:t.c.map((c,i)=>({lane:i+1,class:c})),resultEndpointsIncluded:false,resultIncluded:false,exhibitionIncluded:false}}
...
if(!calibration.length||!calibration[0].m4.races)throw new Error('TODA_CALIBRATION_EMPTY');
...
const report={
  schema:'boat-command-toda-baseline-backtest-v1',version:'TODA-BASELINE-BACKTEST-V1',venue:'TODA',venueCode:'02',source:input,
  ...
  baselineArchitecture:'TODA_CLASS_NEIGHBOR_BASELINE',candidateArchitecture:'TODA_EMPIRICAL_FIRST_SECOND_TRANSITION_PLUS_CLASS_NEIGHBORS',
  ...
  promotionBlockers:['TODA_FORWARD_36_RACES_NOT_READY','TODA_FORWARD_60_RACES_NOT_READY','TODA_FORWARD_MODEL_UPLIFT_NOT_READY']
};
