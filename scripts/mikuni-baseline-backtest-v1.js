#!/usr/bin/env node
'use strict';
const fs=require('fs'),model=require('../mikuni-research-model-v1.js');
const input=process.argv[2]||'mikuni-history-bootstrap-v1.json',output=process.argv[3]||'mikuni-baseline-backtest-v1.json';
const db=JSON.parse(fs.readFileSync(input,'utf8'));if(db.venueCode!=='10')throw new Error('MIKUNI_ONLY');
...
function programFrom(t){return{venue:'MIKUNI',venueCode:'10',date:t.d,race:Number(t.r),raceType:String(t.t||''),boats:t.c.map((c,i)=>({lane:i+1,class:c})),resultEndpointsIncluded:false,resultIncluded:false,exhibitionIncluded:false}}
...
if(!calibration.length||!calibration[0].m4.races)throw new Error('MIKUNI_CALIBRATION_EMPTY');
...
const report={
  schema:'boat-command-mikuni-baseline-backtest-v1',version:'MIKUNI-BASELINE-BACKTEST-V1',venue:'MIKUNI',venueCode:'10',source:input,
  ...
  baselineArchitecture:'MIKUNI_CLASS_NEIGHBOR_BASELINE',candidateArchitecture:'MIKUNI_EMPIRICAL_FIRST_SECOND_TRANSITION_PLUS_CLASS_NEIGHBORS',
  ...
  promotionBlockers:['MIKUNI_FORWARD_36_RACES_NOT_READY','MIKUNI_FORWARD_60_RACES_NOT_READY','MIKUNI_FORWARD_MODEL_UPLIFT_NOT_READY']
};
