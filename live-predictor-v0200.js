// BOAT COMMAND GAMAGORI LIVE CANDIDATE PREDICTOR v0.29.1
// Verified PRE-RACE/exhibition only. No result fetch. No auto bet.
const BC_LIVE_PREDICTOR_V0200={version:'GAMAGORI-LIVE-V0.29.1'};
...
function bcLiveScoreRows(r){
 ...
 return out.sort((a,b)=>b.score-a.score||a.lane-b.lane);
}
function bcLiveCandidate(r){
 if(r?.liveDataStatus!=='READY'||!r?.livePreRace)return{status:'WAIT',reason:r?.liveDataReason||'展示未公開'};
 const rows=bcLiveScoreRows(r);if(rows.length!==6)return{status:'WAIT',reason:'展示は検出したが6艇すべての展示・ST・艇番対応を安全に確認できないため再同期待ち'};
 const [a,b,c,d]=rows.map(x=>x.lane),picks=[...new Set([`${a}-${b}-${c}`,`${a}-${c}-${b}`,`${b}-${a}-${c}`,`${a}-${b}-${d}`])].slice(0,4);
 return{status:'CANDIDATE',stage:'SECOND',picks,rank:rows.map(x=>x.lane),rationale:`LIVE VERIFIED第二候補。評価順 ${rows.map(x=>x.lane).join('→')}。展示タイム・ST・級別・コース優位のみで採点。当日結果・払戻は不使用。`,generatedAt:new Date().toISOString(),strategyVersion:BC_LIVE_PREDICTOR_V0200.version};
}
