// BOAT COMMAND GAMAGORI LIVE PREDICTION COMPACT UI v0.26.1
// UI-only layer. Does not alter prediction, LOCK, result, settlement, or PRE/POST data rules.
(()=>{
  'use strict';
  const VERSION='GAMAGORI-LIVE-COMPACT-V0.26.1';
...
#predictionList.bc-live-compact-v0261 .stake{font-size:13px;opacity:.8}
#predictionList.bc-live-compact-v0261 .two-stage-v0260{display:grid;gap:8px;margin:0}
#predictionList.bc-live-compact-v0261 .two-stage-v0260 .prediction-gate,
#predictionList.bc-live-compact-v0261 .two-stage-v0260 .no-prediction-panel{margin:0;padding:11px 12px;border-radius:12px}
#predictionList.bc-live-compact-v0261 .two-stage-v0260 .prediction-gate b,
#predictionList.bc-live-compact-v0261 .two-stage-v0260 .no-prediction-head b{font-size:12px;opacity:.78}
#predictionList.bc-live-compact-v0261 .two-stage-v0260 .prediction-gate span,
#predictionList.bc-live-compact-v0261 .two-stage-v0260 .no-prediction-head span{display:block;margin-top:4px;font-size:17px;font-weight:850;line-height:1.35}
...
#predictionList.bc-live-compact-v0261 .reason-box,
#predictionList.bc-live-compact-v0261 .two-stage-v0260 .snapshot-note{display:none!important}
...
#predictionList.bc-live-compact-v0261 .race-card.compact-expanded .reason-box,
#predictionList.bc-live-compact-v0261 .race-card.compact-expanded .two-stage-v0260 .snapshot-note{display:revert!important}
...
@media(max-width:720px){
  #predictionList.bc-live-compact-v0261 .race-card{padding:12px}
  #predictionList.bc-live-compact-v0261 .two-stage-v0260 .prediction-gate span,
  #predictionList.bc-live-compact-v0261 .two-stage-v0260 .no-prediction-head span{font-size:16px}
}`;
  ...
  function apply(){
    ...
    const host=document.querySelector('#predictionList');
    if(!host)return;
    const live=!!s&&s.runType==='LIVE';
    ...
    host.querySelectorAll('.race-card').forEach(card=>{
      ...
      if(!tools){
        tools=document.createElement('div');tools.className='compact-tools compact-tools-v0261';
        tools.innerHTML='<button type="button" class="compact-details-btn">詳細を見る</button>';
        card.appendChild(tools);
      }
      const btn=tools.querySelector('.compact-details-btn');
      if(btn)btn.textContent=card.classList.contains('compact-expanded')?'詳細を閉じる':'詳細を見る';
    });
  }
  document.addEventListener('click',e=>{
    const btn=e.target?.closest?.('.compact-details-btn');
    if(!btn)return;
    const card=btn.closest('.race-card');if(!card)return;
    card.classList.toggle('compact-expanded');
    btn.textContent=card.classList.contains('compact-expanded')?'詳細を閉じる':'詳細を見る';
  });
  const priorRender=typeof renderAll==='function'?renderAll:null;
  if(priorRender){renderAll=function(){const out=priorRender.apply(this,arguments);apply();return out;};}
  const priorSweep=typeof sweepVerifiedLiveRelays==='function'?sweepVerifiedLiveRelays:null;
  if(priorSweep){sweepVerifiedLiveRelays=async function(opts={}){const out=await priorSweep(opts);apply();return out;};}
  window.BOAT_COMMAND_LIVE_COMPACT_V0261=Object.freeze({version:VERSION,venue:'蒲郡',uiOnly:true});
  apply();
})();
