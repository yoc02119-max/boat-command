// BOAT COMMAND GAMAGORI LIVE PREDICTION COMPACT UI v0.29.1
// LIVE prediction screen: first/second predictions + HARD LOCK + prediction/app-version refresh.
// Safety: compact mode never hides explicit LIVE WAIT fail-closed gates.
(()=>{
'use strict';
const VERSION='GAMAGORI-LIVE-COMPACT-V0.29.1';
...
async function remoteAssetState(){
  ...
  const required=['live-program-v0270.js','live-predictor-v0200.js','live-two-stage-v0260.js','live-prediction-compact-v0261.js'];
  const sources=[...doc.scripts].map(s=>s.getAttribute('src')||'');
  return {key,stackReady:required.every(name=>sources.some(src=>src.startsWith(name)))};
}
...
async function refreshLatest(){
  ...
  try{
    ...
    if(remote.key!=='unknown'&&(remote.key!==localKey||!remote.stackReady||typeof window.syncGamagoriProgramSnapshot!=='function')){
    ...
    if(typeof BOAT_COMMAND_TWO_STAGE_V0260?.ensureFirstSuggestion==='function')await BOAT_COMMAND_TWO_STAGE_V0260.ensureFirstSuggestion({render:false});
    ...
    if(typeof BOAT_COMMAND_TWO_STAGE_V0260?.ensureLiveSuggestion==='function')await BOAT_COMMAND_TWO_STAGE_V0260.ensureLiveSuggestion({render:false});
    if(typeof BOAT_COMMAND_TWO_STAGE_V0260?.sync==='function')BOAT_COMMAND_TWO_STAGE_V0260.sync();
    if(typeof renderAll==='function')renderAll();
    const post=await remoteAssetState();
    const postKey=currentAssetKey();
    stamp(post.key===postKey&&post.stackReady?`✓ 最新 v${postKey}`:`要再読込 local ${postKey} / remote ${post.key}`);
