...
for(const f of must){if(!fs.existsSync(f))throw new Error(`MISSING_FILE:${f}`);if(!html.includes(`src="${f}?v=338"`))throw new Error(`NOT_LOADED:${f}`)}
let pos=-1;for(const f of must){const p=html.indexOf(`src="${f}?v=338"`);if(p<=pos)throw new Error(`LOAD_ORDER:${f}`);pos=p}
