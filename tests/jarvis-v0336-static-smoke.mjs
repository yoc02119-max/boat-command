...
const must=['boat-command-agent-v0335.js','boat-jarvis-conversation-v0336.js','boat-jarvis-core-v0336.js','boat-jarvis-app-controller-v0336.js','boat-jarvis-development-bridge-v0336.js','boat-jarvis-executor-contract-v0336.js','boat-jarvis-runtime-v0336.js','boat-jarvis-ui-v0336.js'];
const html=fs.readFileSync('index.html','utf8');
for(const f of must){if(!fs.existsSync(f))throw new Error(`MISSING_FILE:${f}`);if(!html.includes(`src="${f}?v=336"`))throw new Error(`NOT_LOADED:${f}`)}
let pos=-1;for(const f of must){const p=html.indexOf(`src="${f}?v=336"`);if(p<=pos)throw new Error(`LOAD_ORDER:${f}`);pos=p}
