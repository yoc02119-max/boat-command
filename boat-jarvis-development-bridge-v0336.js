...
async function call(body){const cloud=window.BOAT_JARVIS_CLOUD_V0337;if(!cloud?.configured?.())throw new Error('JARVIS_CLOUD_NOT_CONFIGURED');return cloud.request('/api/jarvis/github',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)})}
async function dispatch(job){if(!job)return null;try{mark(job.id,'DISPATCHING');const r=await call({action:'dispatch_development',job}),data=await r.json();if(!r.ok){mark(job.id,'FAILED',{error:data?.error||`HTTP_${r.status}`});return {ok:false,error:data?.error||`HTTP_${r.status}`,jobId:job.id};}mark(job.id,'QUEUED_REMOTE',data);try{window.dispatchEvent(new CustomEvent('boat-jarvis-development-dispatched',{detail:data}))}catch{}startPolling();return data;}catch(e){const code=String(e?.message||e),status=code==='JARVIS_CLOUD_NOT_CONFIGURED'||code==='JARVIS_ACCESS_REQUIRED'?'CONNECTION_REQUIRED':'FAILED';mark(job.id,status,{error:code});return {ok:false,error:code,jobId:job.id};}}
async function check(job){try{const r=await call({action:'development_result',jobId:job.id}),data=await r.json();if(!r.ok||!data?.result)return null;const result=data.result;const status=result.status==='COMPLETED'?'COMPLETED':result.status==='FAILED'?'FAILED':'RUNNING';mark(job.id,status,result);try{window.BOAT_JARVIS_EXECUTOR_CONTRACT_V0336?.acceptResult?.(result)}catch{}try{window.dispatchEvent(new CustomEvent('boat-jarvis-development-result',{detail:result}))}catch{}return result;}catch{return null}}
...
window.BOAT_JARVIS_DEVELOPMENT_BRIDGE_V0336=Object.freeze({version:VERSION,enqueue,dispatch,check,poll,startPolling,latest,pending,mark,target:'CODEX_GITHUB',opensExternalPage:false,remoteDispatch:true,resultPolling:true,autoMainPromotion:false});
})();
