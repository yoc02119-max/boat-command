...
function answer(q){const t=String(q||'').trim();const a=t.replace(/\s/g,'');const st=stats();if(!st.session)return '蒲郡LIVEセッションを確認できません。';
 let m=a.match(/(\d{1,2})R/);const race=m?Number(m[1]):null;
 ...
 if(/最新版|バージョン/.test(a))return `BOAT COMMANDは画面表示上 v0.27.7 です。最新に更新ボタンで予想と公開資産を再同期できます。`;
 if(/次の開催日/.test(a))return '次の開催日は、公開済みの開催情報を確認して回答する機能へ接続中です。';
 if(/更新|同期|最新/.test(a))return '__REFRESH__';
 return `蒲郡担当です。「今日どう？」「1Rの第一候補」「1Rの第二候補」「資金」「ロック状況」「最新版」などを聞けます。`;
}
...
function install(){const card=document.querySelector('.assistant-card');const row=card?.querySelector('.input-row');if(!card||!row||document.querySelector('#bcVoiceBtn'))return;
