// BOAT COMMAND GAMAGORI VOICE AGENT v0.28.3
...
async function doRefresh(){
 try{
  const api=window.BOAT_COMMAND_MANUAL_REFRESH_V0276;
  if(typeof api?.refresh==='function'){await api.refresh({user:true});return '予想データとアプリ最新版を同期しました。'}
  const btn=document.querySelector('#bcManualRefreshBtn');if(btn){btn.click();return '予想データとアプリ最新版の同期を開始しました。'}
  if(typeof window.sweepVerifiedLiveRelays==='function'){await window.sweepVerifiedLiveRelays({render:true});return '最新データを同期しました。'}
 }catch(e){console.warn('[voice-refresh]',e);return '更新中にエラーが発生しました。'}
 return '更新機能を確認できませんでした。';
}
function answer(q){const t=String(q||'').trim();const a=t.replace(/\s/g,'');const st=stats();if(!st.session)return '蒲郡LIVEセッションを確認できません。';
 ...
 if(/最新版|バージョン/.test(a))return `現在の公開画面を確認しています。「更新して」と言えば予想とアプリ最新版をまとめて同期します。`;
 if(/更新|同期|最新/.test(a))return '__REFRESH__';
 return `その質問は自由会話側で考えます。「ChatGPTと接続」を押すと、現在の12R状態を持ったままChatGPTへつながります。`;
}
...
window.BOAT_COMMAND_VOICE_AGENT=Object.freeze({version:VERSION,free:true,paidApi:false,handle,refresh:doRefresh});
