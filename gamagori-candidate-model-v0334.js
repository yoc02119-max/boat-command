// BOAT COMMAND GAMAGORI Candidate V2 v0.33.4
// SHADOW ONLY. Pure PRE-RACE model: no fetch, no result access, no exhibition.
(function(root,factory){
  const api=factory();
  if(typeof module==='object'&&module.exports)module.exports=api;
  root.BOAT_COMMAND_GAMAGORI_CANDIDATE_V0334=Object.freeze(api);
})(typeof globalThis!=='undefined'?globalThis:this,function(){
'use strict';
const VERSION='GAMAGORI-CANDIDATE-V0.33.4';
const CLASS_SCORE={A1:3,A2:2,B1:1,B2:0};
// Lane prior intentionally softer than production v0.32.0; lane 1 remains advantaged.
...
function relativeBoatScores(program){
 ...
 return profiles.map((p,i)=>{
   ...
   if(finite(p.racerWinRate))score+=.68*(num(p.racerWinRate)-natMean);
   if(finite(p.localWinRate))score+=.22*(num(p.localWinRate)-localMean);
   if(finite(p.motor2Rate))score+=.48*(num(p.motor2Rate)-motorMean);
   // Smaller ST is stronger; use field-relative difference, never exhibition ST.
...
function predict(program,{count=4}={}){
 ...
 return {version:VERSION,shadowOnly:true,exhibitionUsed:false,resultDataUsed:false,boatScores:scores,rows,fixed,confidence:{top1,top4,gap}};
}
return {version:VERSION,orders:ORDERS,relativeBoatScores,predict};
});
