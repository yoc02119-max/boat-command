// BOAT COMMAND TODA research model v1
// Venue-specific research only. Uses strictly past TODA history and never fetches results.
(function(root,factory){
  const api=factory();
  if(typeof module==='object'&&module.exports)module.exports=api;
  root.BOAT_COMMAND_TODA_RESEARCH_MODEL_V1=Object.freeze(api);
})(typeof globalThis!=='undefined'?globalThis:this,function(){
  'use strict';
  const VERSION='TODA-RESEARCH-MODEL-V1';
  ...
  function classes(program){
    ...
    if(b.length!==6||!b.every((x,i)=>Number(x.lane)===i+1&&Object.hasOwn(CLASS,String(x.class))))throw new Error('TODA_PROGRAM_INVALID');
  ...
  function distribution(program,history,{targetDate=null,config=null}={}){
    if(program?.venue!=='TODA'||String(program?.venueCode)!=='02')throw new Error('TODA_ONLY');
    if(program.resultEndpointsIncluded!==false||program.resultIncluded!==false||program.exhibitionIncluded!==false)throw new Error('TODA_PROGRAM_BOUNDARY');
    const c=classes(program),cfg={...DEFAULT,...(config||{})};
    const safe=(history||[]).filter(x=>validOrder(x.o)&&Array.isArray(x.c)&&x.c.length===6&&(!targetDate||String(x.d)<String(targetDate))).slice(-Math.max(300,Number(cfg.recencyWindow)||300));
    if(safe.length<300)throw new Error('TODA_HISTORY_MIN_300_REQUIRED');
    ...
    const base=new Map();
    for(const o of ORDERS){const [a,b,z]=o.split('-').map(Number),p2=trans.get(a),rem=td-tr[a]-tr[b];base.set(o,(f[a]/fd)*(p2.v[b]/p2.den)*(rem>0?tr[z]/rem:0))}
    ...
    const local=new Map();let mass=0;for(const n of nearest){const w=Math.exp(-(Number(cfg.decay)||.55)*n.d);local.set(n.x.o,(local.get(n.x.o)||0)+w);mass+=w}
    const mix=Math.max(0,Math.min(1,Number(cfg.neighborMix)||0));
    const rows=ORDERS.map(order=>({order,probability:(1-mix)*(base.get(order)||0)+mix*(mass>0?(local.get(order)||0)/mass:0)}));
    const sum=rows.reduce((s,x)=>s+x.probability,0)||1;for(const x of rows)x.probability/=sum;rows.sort((a,b)=>b.probability-a.probability);
    return{version:VERSION,rows,sum:rows.reduce((s,x)=>s+x.probability,0),historyRows:safe.length,nearestDistance:nearest[0]?.d??null,config:cfg,architecture:'TODA_EMPIRICAL_FIRST_SECOND_TRANSITION_PLUS_CLASS_NEIGHBORS',resultInput:false,payoutInput:false,researchOnly:true,productionEnabled:false,tryEnabled:false};
  }
