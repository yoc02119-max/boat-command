...
(function(root,factory){
  ...
  function distribution(program,history,{targetDate=null,config=null,mode='PROGRAM_ONLY'}={}){
    ...
    if(!['CLASS_BASELINE','PROGRAM_ONLY'].includes(mode))throw new Error('TODA_MODE_INVALID');
    ...
    if(!(mass>0))throw new Error('TODA_NEIGHBOR_MASS_EMPTY');

    let raw;
    let architecture;
    if(mode==='CLASS_BASELINE'){
      raw=ORDERS.map(order=>({order,probability:(local.get(order)||0)/mass}));
      architecture='TODA_CLASS_NEIGHBOR_BASELINE';
    }else{
