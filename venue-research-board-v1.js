...
(function(root,factory){
  ...
  async function mount(opts){
    ...
    const cancelled=dayStatus?.status==='CANCELLED'||venueCalendar?.todayCancelled===true;
    const cancelledRefunded=cancelled&&dayStatus?.bankrollPolicy==='RELEASE_RESERVED_STAKE_ONLY';
    const cancelledMessage=cancelledRefunded
      ?'本日は中止・順延です。確定結果はありません。対象TRYは返還処理済みです。'
      :'本日は中止・順延です。確定結果はありません。';
