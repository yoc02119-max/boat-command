...
export function cors(req,res,methods='POST, OPTIONS'){
  ...
  const ownVercelOrigin=/^[a-z0-9.-]+\.vercel\.app(?::\d+)?$/i.test(host)?`https://${host}`:'';
  const allowed=origin===ALLOWED_ORIGIN||Boolean(ownVercelOrigin&&origin===ownVercelOrigin);
  if(allowed){res.setHeader('Access-Control-Allow-Origin',origin);res.setHeader('Access-Control-Allow-Headers','Authorization, Content-Type');res.setHeader('Access-Control-Allow-Methods',methods)}
