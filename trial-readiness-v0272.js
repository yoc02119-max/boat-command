// BOAT COMMAND GAMAGORI LIVE TRIAL READINESS v0.31.3
(()=>{
'use strict';
const VERSION='GAMAGORI-LIVE-TRIAL-READINESS-V0.31.3';
const REQUIRED_IDS=['sessionDate','modeBadge','runType','predictionList','resultGate','resultSummary','resultList','raceStrip'];
const REQUIRED_SCRIPTS=['app.js','virtual-bankroll-v0250.js','gamagori-live-integration-v0198.js','live-program-v0270.js','live-autopoll-v0199.js','live-predictor-v0200.js','live-two-stage-v0260.js','live-prediction-compact-v0261.js','live-skip-gate-v0225.js','live-autofill-v0201.js','live-lock-guard-v0202.js','live-lock-snapshot-v0203.js','live-result-v0204.js','live-learning-v0205.js','live-learning-dashboard-v0206.js','live-learning-guard-v0207.js','live-integrity-audit-v0208.js','live-snapshot-hash-guard-v0209.js','development-status-v0217.js','trial-readiness-v0218.js','live-session-today-v0263.js','live-manual-refresh-v0276.js','trial-readiness-v0272.js','boat-ai-core-v0290.js','boat-voice-agent-v0296.js','boat-chatgpt-voice-v0298.js','boat-ai-visual-v0312.js','boat-remote-control-v0284.js'];
...
function audit(){
 ...
 const firstVisible=currentLive?[...document.querySelectorAll('#predictionList .bc-main-prediction')].filter(x=>/\d-\d-\d/.test(x.textContent||'')).length:null;
 ...
 const operationOk=!currentLive||(programReady===12&&firstReady===12&&firstVisible===12);
 ...
 if(currentLive&&firstVisible!==12)holdReasons.push(`第一候補表示 ${firstVisible}/12`);
 return Object.freeze({version:VERSION,venue:'蒲郡',status:ok?'READY':'HOLD',ok,structuralOk,operationOk,idsMissing,scriptsMissing,duplicates,scriptOrderOk:orderOk,cacheContractOk:cacheOk,cacheKey:keys.find(Boolean)||null,functionsMissing,globals,currentLive,programReady,programWait,firstStageReady:firstReady,firstStageVisible:firstVisible,holdReasons,prePostSeparated:true,waitFailClosed:true,noResultLookahead:true,checkedAt:new Date().toISOString()});
}
function render(out){
 ...
 const visible=out.firstStageVisible===null?'—':`${out.firstStageVisible}/12`;
 const reason=out.ok?'運営条件OK':out.holdReasons.join(' / ')||'確認待ち';
 box.textContent=`${out.ok?'🟢':'🟡'} LIVE readiness ${out.status} · PROGRAM ${out.programReady}/12 · 第一候補 ${first} · 表示 ${visible} · ${reason} · cache v${out.cacheKey||'?'} · ${new Intl.DateTimeFormat('ja-JP',{timeZone:'Asia/Tokyo',hour:'2-digit',minute:'2-digit',second:'2-digit'}).format(new Date())}`;
}
