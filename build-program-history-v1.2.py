#!/usr/bin/env python3
"""BOAT COMMAND · PROGRAM HISTORY BUILDER v1.2
...
VERSION = "PROGRAM-HISTORY-BUILDER-V1.2"
...
RACE_HEAD = re.compile(r"^\s*(\d{1,2})\s*[RＲ]\s*(.*?)\s+[HＨ]\s*1800", re.I)
...
def parse_program_block(block: str) -> dict[int, ProgramRace]:
    ...
    for line in block.splitlines():
        ...
        if h:
            ...
            race_type = normalize_type(h.group(2))
            classes = {}
            continue
        if current is None:
            continue
...
def self_test() -> None:
    ...
    print("SELF-TEST PASS · program/result parser")
