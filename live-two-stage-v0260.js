// BOAT COMMAND GAMAGORI TWO-STAGE PREDICTION v0.26.5
(()=>{
'use strict';
const VERSION='GAMAGORI-TWO-STAGE-V0.26.5';
...
function fallbackProgramClasses(r){
  ...
  const p=r?.preRaceProfiles;
  if(Array.isArray(p)&&p.length===6)return p.map(x=>String(x?.cls||x?.class||'B1'));
  return null;
}
...
function sessionHistory(){try{return Object.values(typeof store!=='undefined'&&store?.sessions?store.sessions:{})}catch{return []}}
function firstCandidate(s,r){
  ...
  const comp=currentComposition(s,r);if(!comp)return{status:'WAIT',reason:'当日番組データ待ち',sessionDate:s.date};
  const scores=new Map();let samples=0;
  for(const hs of sessionHistory()){
    if(!hs?.date||String(hs.date)>=String(s.date))continue;
    for(const hr of hs.races||[]){
      if(!hr?.settled)continue;const res=validResult(hr.result);if(!res)continue;
      let hc=hr.programComposition||null;if(!hc){const classes=fallbackProgramClasses(hr);if(classes)hc={classes,type:classes.join('-'),fingerprint:classes.join('|')}}
      if(!hc?.classes||hc.classes.length!==6)continue;
      let sim=.15;if(hc.type===comp.type)sim+=1.2;for(let i=0;i<6;i++)if(hc.classes[i]===comp.classes[i])sim+=.32;if(hc.fingerprint===comp.fingerprint)sim+=2.5;
      scores.set(res,(scores.get(res)||0)+sim);samples++;
    }
  }
  if(!scores.size)return{status:'WAIT',reason:'過去番組データ蓄積待ち',sessionDate:s.date};
  const picks=[...scores].sort((a,b)=>b[1]-a[1]).slice(0,4).map(x=>x[0]),now=new Date().toISOString();
  return{status:'CANDIDATE',stage:'FIRST',picks,samples,sessionDate:s.date,generatedAt:now,lockedAt:now,strategyVersion:VERSION,rationale:`${s.date}の結果・展示は不使用。過去の類似番組${samples}件から照合。`};
}
...
const firstHtml=r=>{const x=r?.firstSuggestion;if(x?.status!=='CANDIDATE')return `<div class="bc-stage-row bc-first"><div class="bc-stage-top"><b>第一候補</b><span class="bc-stage-tag">過去データ</span></div><div class="bc-stage-status">${safeEsc(x?.reason||'準備中')}</div></div>`;return `<div class="bc-stage-row bc-first"><div class="bc-stage-top"><b>第一候補</b><span class="bc-stage-tag">過去データ 🔒</span></div><div class="bc-stage-picks">${x.picks.map(safeEsc).join(' / ')}</div><details class="bc-stage-detail"><summary>理由を見る</summary>${safeEsc(x.rationale)}</details></div>`};
const secondHtml=(s,r)=>{const x=r?.liveSuggestion;if(!x||x.status==='WAIT')return `<div class="bc-stage-row bc-second wait"><div class="bc-stage-top"><b>第二候補</b><span class="bc-stage-tag">展示後</span></div><div class="bc-stage-status">${safeEsc(x?.reason||'展示待ち')}</div></div>`;if(x.status==='SKIP')return `<div class="bc-stage-row bc-second wait"><div class="bc-stage-top"><b>第二候補</b><span class="bc-stage-tag">なし</span></div><div class="bc-stage-status">${safeEsc(x.reason||'直前情報不足')}</div></div>`;return `<div class="bc-stage-row bc-second ready"><div class="bc-stage-top"><b>第二候補</b><span class="bc-stage-tag">展示反映 🔒</span></div><div class="bc-stage-picks">${(x.picks||[]).map(safeEsc).join(' / ')}</div></div>`};
...
window.addEventListener('boatcommand:today-live',()=>{setTimeout(sync,0);setTimeout(()=>{if(typeof sweepVerifiedLiveRelays==='function')sweepVerifiedLiveRelays({render:true})},80)});
window.BOAT_COMMAND_TWO_STAGE_V0260=Object.freeze({version:VERSION,venue:'蒲郡',sync});installSimpleUi();sync();
})();