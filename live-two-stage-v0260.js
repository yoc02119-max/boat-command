// BOAT COMMAND GAMAGORI TWO-STAGE PREDICTION v0.29.1
...
(()=>{
'use strict';
const VERSION='GAMAGORI-TWO-STAGE-V0.29.1';
...
function programInfo(r){
  ...
  if(Array.isArray(boats)&&boats.length===6){
    ...
    if(sorted.every((b,i)=>Number(b.lane)===i+1)){
      const classes=sorted.map(b=>String(b.class||'B1'));
      return {available:true,classes,type:classes.join('-'),fingerprint:sorted.map(b=>[Number(b.lane),String(b.class||''),String(b.motor??''),String(b.boat??'')].join(':')).join('|'),profiles:sorted,source:'verified-pre'};
    }
  }
  return null;
}
...
const secondHtml=(s,r)=>{const x=r?.liveSuggestion;if(!x||x.status==='WAIT')return `<div class="bc-stage-row bc-second wait"><div class="bc-stage-top"><b>第二候補</b><span class="bc-stage-tag">展示後</span></div><div class="bc-stage-status">${safeEsc(x?.reason||'展示待ち')}</div></div>`;if(x.status==='SKIP')return `<div class="bc-stage-row bc-second wait"><div class="bc-stage-top"><b>第二候補</b><span class="bc-stage-tag">安全停止</span></div><div class="bc-stage-status">${safeEsc(x.reason||'直前情報不足')}</div></div>`;return `<div class="bc-stage-row bc-second ready"><div class="bc-stage-top"><b>第二候補</b><span class="bc-stage-tag">展示反映 🔒</span></div><div class="bc-stage-picks">${(x.picks||[]).map(safeEsc).join(' / ')}</div><details class="bc-stage-detail"><summary>理由を見る</summary>${safeEsc(x.rationale||'verified展示データを反映')}</details></div>`};
...
function sync(){refresh();if(typeof refreshLiveCandidates==='function')refreshLiveCandidates();render()}
...
window.BOAT_COMMAND_TWO_STAGE_V0260=Object.freeze({version:VERSION,venue:'蒲郡',sync,ensureHistory,ensureFirstSuggestion:async({render:doRender=true}={})=>{refresh();if(doRender)render();return true},ensureLiveSuggestion:async({render:doRender=true}={})=>{if(typeof refreshLiveCandidates==='function')refreshLiveCandidates();if(doRender)render();return true}});installSimpleUi();ensureHistory();sync();
})();