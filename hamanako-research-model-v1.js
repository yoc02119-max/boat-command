// BOAT COMMAND HAMANAKO research model v1
// Venue-specific research only. Uses strictly past HAMANAKO history and never fetches results.
(function(root,factory){
  const api=factory();
  if(typeof module==='object'&&module.exports)module.exports=api;
  root.BOAT_COMMAND_HAMANAKO_RESEARCH_MODEL_V1=Object.freeze(api);
})(typeof globalThis!=='undefined'?globalThis:this,function(){
  'use strict';
  const VERSION='HAMANAKO-RESEARCH-MODEL-V1';
  ...
  function classes(program){
    ...
    if(b.length!==6||!b.every((x,i)=>Number(x.lane)===i+1&&Object.hasOwn(CLASS,String(x.class))))throw new Error('HAMANAKO_PROGRAM_INVALID');
  ...
  function distribution(program,history,{targetDate=null,config=null,mode='PROGRAM_ONLY'}={}){
    if(program?.venue!=='HAMANAKO'||String(program?.venueCode)!=='06')throw new Error('HAMANAKO_ONLY');
    if(program.resultEndpointsIncluded!==false||program.resultIncluded!==false||program.exhibitionIncluded!==false)throw new Error('HAMANAKO_PROGRAM_BOUNDARY');
    if(!['CLASS_BASELINE','PROGRAM_ONLY'].includes(mode))throw new Error('HAMANAKO_MODE_INVALID');
    ...
    if(safe.length<300)throw new Error('HAMANAKO_HISTORY_MIN_300_REQUIRED');
    ...
    if(!(mass>0))throw new Error('HAMANAKO_NEIGHBOR_MASS_EMPTY');

    let raw;
    let architecture;
    if(mode==='CLASS_BASELINE'){
      raw=ORDERS.map(order=>({order,probability:(local.get(order)||0)/mass}));
      architecture='HAMANAKO_CLASS_NEIGHBOR_BASELINE';
    }else{
      ...
      architecture='HAMANAKO_EMPIRICAL_FIRST_SECOND_TRANSITION_PLUS_CLASS_NEIGHBORS';
    }
