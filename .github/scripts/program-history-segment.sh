...
d=json.loads(Path("gamagori-2025-schedule-manifest.json").read_text())["dates"]
...
while IFS= read -r d; do
  ...
  python3 program-history-builder-v121.py --b-dir segment-work/one/b --k-dir segment-work/one/k --out segment-work/one/out --venue 07 >/dev/null 2>&1 || { echo "$d BUILDER_FAIL" >> segment-work/exceptions.txt; continue; }
  python3 - "$d" <<'PY' || { echo "$d NOT_12R_COMPLETE" >> segment-work/exceptions.txt; continue; }
import json,sys
from pathlib import Path
d=sys.argv[1]
p=Path("segment-work/one/out/07/2025.json")
