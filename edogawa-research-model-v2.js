// BOAT COMMAND EDOGAWA research model v2
// Research-only. Venue priors are learned from strictly past EDOGAWA history.
// No fetch, result, payout, bankroll or live-bet access.
(function(root,factory){
  const api=factory();
  if(typeof module==='object'&&module.exports)module.exports=api;
  root.BOAT_COMMAND_EDOGAWA_RESEARCH_MODEL_V2=Object.freeze(api);
})(typeof globalThis!=='undefined'?globalThis:this,function(){
  'use strict';
  const VERSION='EDOGAWA-RESEARCH-MODEL-V2';
  ...
  function learnedLanePrior(history,race,{priorStrength=36,scoreScale=.6}={}){
    const safe=(history||[]).filter(x=>validOrder(x.o));
    if(safe.length<300)throw new Error('EDOGAWA_HISTORY_MIN_300_REQUIRED');
    const global=[0,0,0,0,0,0],local=[0,0,0,0,0,0];
    let localN=0;
    for(const x of safe){
      const lane=Number(String(x.o).split('-')[0]);
      if(lane>=1&&lane<=6)global[lane-1]++;
      if(Number(x.r)===Number(race)){
        if(lane>=1&&lane<=6)local[lane-1]++;
        localN++;
      }
    }
    const globalN=global.reduce((a,b)=>a+b,0);
    const gp=global.map(n=>(n+1)/(globalN+6));
    const probs=local.map((n,i)=>(n+priorStrength*gp[i])/(localN+priorStrength));
    const logs=probs.map(p=>Math.log(Math.max(p,1e-9)));
    const mean=logs.reduce((a,b)=>a+b,0)/6;
    const scores=logs.map(x=>scoreScale*(x-mean));
    return {scores,probabilities:probs,globalProbabilities:gp,localRaceRows:localN,historyRows:globalN,priorStrength,scoreScale};
  }
  function currentScores(program,feature=null,{fullPreRace=false,classOnly=false,lanePrior=null}={}){
    const b=[...program.boats].sort((a,b)=>Number(a.lane)-Number(b.lane));
    const prior=Array.isArray(lanePrior)&&lanePrior.length===6?lanePrior:Array(6).fill(0);
    const lanes=feature?.lanes||[];
    return b.map((x,i)=>{
      let s=prior[i]+.36*CLASS[String(x.class)];
      if(!classOnly){
  ...
  function distribution(program,history,feature=null,{mode='PROGRAM_ONLY',targetDate=null,neighborLimit=360,decay=.55,priorMass=36}={}){
    ...
    const full=mode==='FULL_PRE_RACE',classOnly=mode==='CLASS_BASELINE';
    if(full&&!feature?.preRaceComplete)throw new Error('FULL_PRE_RACE_NOT_READY');

    const lanePrior=learnedLanePrior(safe,program.race);
    const scores=currentScores(program,feature,{fullPreRace:full,classOnly,lanePrior:lanePrior.scores});
    ...
    return{
      ...
      laneScores:scores,lanePrior,
      lanePriorSource:'STRICT_PAST_EDOGAWA_HISTORY_BY_RACE_NUMBER',
  ...
  return Object.freeze({
    version:VERSION,orders:ORDERS,validOrder,raceTypeGroup,historyDistance,learnedLanePrior,currentScores,distribution,select,
    lanePriorSource:'STRICT_PAST_EDOGAWA_HISTORY_BY_RACE_NUMBER',
