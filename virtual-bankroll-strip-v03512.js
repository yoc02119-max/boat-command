// BOAT COMMAND GAMAGORI virtual bankroll strip v0.35.12
// Display-only summary for the simple 12R screen. Real money is never touched.
(()=>{'use strict';
const VERSION='GAMAGORI-VIRTUAL-BANKROLL-STRIP-V0.35.12';
...
function render(){
 ...
 const invested=locked.reduce((sum,r)=>sum+(Number(r?.stake)||0),0);
 const settled=locked.filter(r=>r?.settled);
 const profit=settled.reduce((sum,r)=>sum+(Number(r?.profit)||0),0);
 const pending=locked.filter(r=>!r?.settled).reduce((sum,r)=>sum+(Number(r?.stake)||0),0);
 ...
 el.innerHTML=`<div class="bankroll"><small>仮資金</small><b>${yen(bankroll)}</b></div><div><small>本日投入</small><b>${yen(invested)}</b></div><div class="profit ${cls}"><small>確定損益</small><b>${profit>0?'+':''}${yen(profit)}</b></div><div class="sub">仮想運用のみ · 未精算LOCK ${yen(pending)} は仮資金から差引済み</div>`;
}
...
window.BOAT_COMMAND_VIRTUAL_BANKROLL_STRIP_V03512=Object.freeze({version:VERSION,render,realMoney:false});
