...
(()=>{'use strict';
  ...
  function relativeRepoDataPath(input){
    ...
    if(raw.startsWith('./')) {
      ...
      if(rel.startsWith('live/')||rel.startsWith('venues/')||rel==='shared-try-portfolio-v1.json'||rel.startsWith('shared-try-portfolio-v1.json?')||rel==='venue-calendar-v1.json'||rel.startsWith('venue-calendar-v1.json?')||rel==='venue-model-cycle-fleet-v1.json'||rel.startsWith('venue-model-cycle-fleet-v1.json?'))return rel;
      return null;
    }

    // Some callers may omit "./".
    if(raw.startsWith('live/')||raw.startsWith('venues/')||raw.startsWith('shared-try-portfolio-v1.json')||raw.startsWith('venue-calendar-v1.json')||raw.startsWith('venue-model-cycle-fleet-v1.json'))return raw;
    ...
    try{
      ...
      if(!(p.startsWith('live/')||p.startsWith('venues/')||p==='shared-try-portfolio-v1.json'||p==='venue-calendar-v1.json'||p==='venue-model-cycle-fleet-v1.json'))return null;
      return p+u.search;
    }catch{return null}
  }
  ...
  window.BOAT_COMMAND_GITHUB_DATA_V1=Object.freeze({
    version:'GITHUB-MAIN-DATA-ROUTER-V1.2',
    rawBase:RAW_BASE,
    rawUrl
  });
})();