// BOAT COMMAND GAMAGORI virtual bankroll home v0.35.13
// Presentation-only home dashboard for simulated funds. No real-money actions.
(()=>{'use strict';
const VERSION='GAMAGORI-VIRTUAL-HOME-V0.35.13';
...
function metrics(){
 ...
 const lockedStake=locked.reduce((a,r)=>a+(Number(r?.stake)||0),0);
 const pendingStake=locked.filter(r=>!r?.settled).reduce((a,r)=>a+(Number(r?.stake)||0),0);
 const settledStake=settled.reduce((a,r)=>a+(Number(r?.stake)||0),0);
 ...
 const hitRate=settled.length?hits/settled.length*100:null;
 const planned=races.filter(r=>!r?.locked&&r?.firstSuggestion?.status==='CANDIDATE')
 ...
 return {s,races,locked,settled,lockedStake,pendingStake,settledStake,returned,profit,hits,roi,hitRate,planned,bankroll,start};
}
function chartSvg(){
...
@media(max-width:760px){#bcVirtualHome{padding:12px}.vh-main,.vh-bottom{grid-template-columns:1fr}.vh-balance strong{font-size:30px}.vh-kpis{grid-template-columns:1fr 1fr}.vh-chart{height:125px}.vh-head h2{font-size:18px}}
`;
...
function render(){
 ...
 <div class="vh-main">
  <div class="vh-balance"><small>現在の仮資金</small><strong>${yen(m.bankroll)}</strong><p>開始 ${yen(m.start)} · 未精算LOCK ${yen(m.pendingStake)} は差引済み</p>
   <div class="vh-kpis"><div><span>本日投入済</span><b>${yen(m.lockedStake)}</b></div><div><span>予定投入</span><b>${yen(m.planned)}</b></div><div class="profit ${pc}"><span>確定損益</span><b>${m.profit>0?'+':''}${yen(m.profit)}</b></div><div><span>ROI</span><b>${pct(m.roi)}</b></div></div>
  </div>
  <div class="vh-graph"><small>仮資金推移</small>${chartSvg()}</div>
 </div>
 <div class="vh-bottom">
  <div class="vh-txs"><div class="vh-title">最近の資金移動</div>${transactions()}</div>
  <div class="vh-actions"><div class="vh-title">今日の状況</div><div class="vh-kpis"><div><span>精算</span><b>${m.settled.length}R</b></div><div><span>的中</span><b>${m.hits}</b></div><div><span>的中率</span><b>${pct(m.hitRate)}</b></div><div><span>未精算</span><b>${m.locked.length-m.settled.length}R</b></div></div><div class="vh-action-grid"><button type="button" data-vh-jump="predict">12Rを見る</button><button type="button" data-vh-jump="results">結果を見る</button></div><div class="vh-note">1点 ${yen(PICK_PRICE)} の仮想運用。予想と結果の境界は従来どおり維持します。</div></div>
 </div>`;
...
window.BOAT_COMMAND_VIRTUAL_HOME_V03513=Object.freeze({version:VERSION,render,realMoney:false});
