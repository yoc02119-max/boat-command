(()=>{
  ...
  const VERSION='GAMAGORI-EARLY-TRIAL-READINESS-V0.21.8+SKIP-GATE-V0.22.5';
  const REQUIRED_IDS=[
  ...
  function audit(){
    ...
    return Object.freeze({
      ...
      note:ok?'画面・LIVE依存・WAIT/SKIP・HARD LOCK安全モジュールの技術的試用条件を満たしています。予想精度の収益性保証ではありません。':'試用導線に不足があります。HOLDのまま使用してください。',
  ...
  function render(out){
    ...
    const detail=out.ok?'LIVE依存・WAIT/SKIP・HARD LOCK・画面契約 OK':`不足 ${out.idsMissing.length+out.scriptsMissing.length+out.functionsMissing.length+out.duplicateScripts.length}件`;
    box.innerHTML=`<div><b>${label}</b><div style="font-size:11px;opacity:.65;margin-top:3px">${detail}</div></div><small style="opacity:.7">v0.21.8+0.22.5 · READ ONLY</small>`;
  }
