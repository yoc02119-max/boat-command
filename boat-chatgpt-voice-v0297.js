// BOAT COMMAND -> CHATGPT VOICE HANDOFF v0.29.6
// Zero-API handoff. Opens ChatGPT immediately from the user's tap, then hands over current state.
(()=>{
'use strict';
const VERSION='GAMAGORI-CHATGPT-VOICE-HANDOFF-V0.29.7';
...
function openChatGPT(ctx){try{return window.open(`https://chatgpt.com/?q=${encodeURIComponent(ctx)}`,'_blank')}catch{return null}}
async function handoff(){
 ...
 const opened=openChatGPT(ctx);
 const copied=await copyText(ctx);
 status(opened?'開いた先で音声ボタンを1回押す':copied?'状態をコピー済み・ChatGPTへ貼り付け':'接続に失敗しました');
 return!!opened
}
function install(){
 ...
 wrap.innerHTML='<button id="bcChatGPTVoiceBtn" type="button">🎙 ChatGPTボイスへ</button><span id="bcChatGPTVoiceStatus">現在状態を自動連携</span>';
...
window.BOAT_COMMAND_CHATGPT_VOICE=Object.freeze({version:VERSION,buildContext,handoff,zeroApi:true,embedded:false,autoStartsVoice:false});
