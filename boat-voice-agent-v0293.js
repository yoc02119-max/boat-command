...
(()=>{
'use strict';
const VERSION='GAMAGORI-VOICE-AGENT-V0.29.3';
...
function speak(text,{test=false}={}){const value=String(text||'').replace(/<[^>]+>/g,'').trim();if(!value||!canSpeak){voiceStatus('音声出力 非対応','error');return false}try{speechSynthesis.cancel();speechSynthesis.resume();const u=new SpeechSynthesisUtterance(value);const v=japaneseVoice();if(v)u.voice=v;u.lang='ja-JP';u.rate=.98;u.pitch=1;u.volume=1;u.onstart=()=>voiceStatus(test?'音声テスト中':'音声で返答中','speaking');u.onend=()=>{activeUtterance=null;voiceStatus('AI LOCAL CORE • 音声待機','ready')};u.onerror=e=>{activeUtterance=null;console.warn('[voice-output]',e);voiceStatus('音声出力エラー・再タップ','error')};activeUtterance=u;setTimeout(()=>{try{speechSynthesis.resume();speechSynthesis.speak(u)}catch(e){console.warn('[voice-speak]',e);voiceStatus('音声出力エラー','error')}},test?20:220);return true}catch(e){console.warn('[voice-output]',e);voiceStatus('音声出力エラー','error');return false}}
...
function installCoreBadge(card){const head=card?.querySelector('.panel-head');if(!head||document.querySelector('#bcVoiceStatus'))return;const badge=document.createElement('span');badge.id='bcVoiceStatus';badge.className='bc-core-status';badge.textContent=window.BOAT_COMMAND_AI_CORE?'AI LOCAL CORE • 音声待機':'LOCAL CORE 読込待ち';head.appendChild(badge)}
function install(){const card=document.querySelector('.assistant-card'),row=card?.querySelector('.input-row');if(!card||!row){loadBridge();return}installCoreBadge(card);if(document.querySelector('#bcVoiceBtn')){loadBridge();return}
 ...
 if(!SR){btn.textContent='🎙 音声入力非対応';btn.disabled=true;voiceStatus(canSpeak?'AI LOCAL CORE • 音声入力非対応':'音声機能 非対応','error');loadBridge();return}
 ...
 rec.onend=()=>{btn.classList.remove('listening');btn.textContent='🎙 話す';const q=pendingTranscript;pendingTranscript='';if(q)setTimeout(()=>handle(q,{spoken:true}),260);else voiceStatus('AI LOCAL CORE • 音声待機','ready')};
...
window.BOAT_COMMAND_VOICE_AGENT=Object.freeze({version:VERSION,free:true,paidApi:false,localCore:true,generativeAi:false,handle,speak,refresh:doRefresh});
