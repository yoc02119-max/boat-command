...
(()=>{
'use strict';
const VERSION='GAMAGORI-VOICE-AGENT-V0.29.4';
...
function speechText(text){
 ...
 v=v.replace(/([1-6])\s*[-→]\s*([1-6])\s*[-→]\s*([1-6])/g,'$1号艇、$2号艇、$3号艇');
...
function speak(text,{test=false}={}){const value=speechText(text);if(!value||!canSpeak){voiceStatus('音声出力 非対応','error');return false}try{speechSynthesis.cancel();speechSynthesis.resume();const u=new SpeechSynthesisUtterance(value);const v=japaneseVoice();if(v)u.voice=v;u.lang='ja-JP';u.rate=.96;u.pitch=1;u.volume=1;u.onstart=()=>voiceStatus(test?'音声テスト中':'音声で返答中','speaking');u.onend=()=>{activeUtterance=null;voiceStatus('AI LOCAL CORE • 音声待機','ready')};u.onerror=e=>{activeUtterance=null;console.warn('[voice-output]',e);voiceStatus('音声出力エラー・再タップ','error')};activeUtterance=u;setTimeout(()=>{try{speechSynthesis.resume();speechSynthesis.speak(u)}catch(e){console.warn('[voice-speak]',e);voiceStatus('音声出力エラー','error')}},test?20:220);return true}catch(e){console.warn('[voice-output]',e);voiceStatus('音声出力エラー','error');return false}}
