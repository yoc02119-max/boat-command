...
let online=false;
function base(){return String(window.BOAT_JARVIS_API_BASE||CONFIG.apiBase||'').trim().replace(/\/$/,'')}
...
async function request(path,options={}){const token=accessToken();if(!token)throw new Error('JARVIS_ACCESS_REQUIRED');const headers={...(options.headers||{}),Authorization:`Bearer ${token}`};const response=await fetch(endpoint(path),{...options,headers,mode:'cors',cache:'no-store',credentials:'omit'});if(response.status===401){clearToken();throw new Error('JARVIS_AUTH_REQUIRED')}if(response.ok)online=true;return response}
