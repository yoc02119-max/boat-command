...
(()=>{'use strict';
const POLICY={version:'GAMAGORI-MAIN-LOCK-GUARD-V0.33.2',minMarginMinutes:3,maxFutureSkewMinutes:2};
...
function audit(s,r,now=new Date()){
 ...
 const rawSnapshotAt=String(r.programSnapshotAt||'').trim();if(!rawSnapshotAt)return {ok:false,status:'BLOCKED',reason:'番組取得時刻未確認'};
 const source=new Date(rawSnapshotAt);if(!Number.isFinite(source.getTime())||(now-source)/60000 < -POLICY.maxFutureSkewMinutes)return {ok:false,status:'BLOCKED',reason:'番組取得時刻異常'};
