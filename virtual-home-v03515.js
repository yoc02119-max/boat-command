// BOAT COMMAND GAMAGORI virtual bankroll home v0.35.15
// Presentation-only home dashboard for simulated funds. No real-money actions.
(()=>{'use strict';
const VERSION='GAMAGORI-VIRTUAL-HOME-V0.35.15';
...
function metrics(){
 ...
 const locked=races.filter(r=>r?.locked);
 const settled=locked.filter(r=>r?.settled);
 const t=typeof bcVirtualTryLedger==='function'?bcVirtualTryLedger():{todayCommittedStakeYen:0,todayPendingStakeYen:0,todaySettledStakeYen:0,todayReturnYen:0,todayProfitYen:0,todayHits:0,todaySettledRaces:0,todayPendingRaces:0,currentTryRaces:[]};
 const tryRaces=new Set(t.currentTryRaces||[]);
 const mainLockedStake=locked.reduce((a,r)=>a+(Number(r?.stake)||0),0);
 const mainPendingStake=locked.filter(r=>!r?.settled).reduce((a,r)=>a+(Number(r?.stake)||0),0);
 const mainSettledStake=settled.reduce((a,r)=>a+(Number(r?.stake)||0),0);
 const mainReturned=settled.reduce((a,r)=>a+(Number(r?.returnAmount)||0),0);
 const lockedStake=mainLockedStake+(Number(t.todayCommittedStakeYen)||0);
 const pendingStake=mainPendingStake+(Number(t.todayPendingStakeYen)||0);
 const settledStake=mainSettledStake+(Number(t.todaySettledStakeYen)||0);
 const returned=mainReturned+(Number(t.todayReturnYen)||0);
 const profit=returned-settledStake;
 const hits=settled.filter(r=>r?.hit).length+(Number(t.todayHits)||0);
 const settledCount=settled.length+(Number(t.todaySettledRaces)||0);
 const pendingCount=locked.filter(r=>!r?.settled).length+(Number(t.todayPendingRaces)||0);
 const roi=settledStake?returned/settledStake*100:null;
 const hitRate=settledCount?hits/settledCount*100:null;
 const planned=races.filter(r=>!r?.locked&&!tryRaces.has(Number(r?.race))&&r?.firstSuggestion?.status==='CANDIDATE')
   .reduce((a,r)=>a+((r?.picks||[]).map(x=>String(x||'').trim()).filter(Boolean).length*PICK_PRICE),0);
 const bankroll=typeof bcVirtualBankrollNow==='function'?bcVirtualBankrollNow():(Number(store?.startBankroll)||100000)+profit-pendingStake;
 const start=(Number(store?.startBankroll)||100000)+(typeof bcVirtualDepositTotal==='function'?bcVirtualDepositTotal():0);
 return {s,races,locked,settled,lockedStake,pendingStake,settledStake,returned,profit,hits,settledCount,pendingCount,roi,hitRate,planned,bankroll,start,tryCommitted:Number(t.todayCommittedStakeYen)||0};
}
function chartSvg(){
...
function render(){
 ...
 <div class="vh-main">
  <div class="vh-balance"><small>現在の仮資金</small><strong>${yen(m.bankroll)}</strong><p>開始 ${yen(m.start)} · 未精算 ${yen(m.pendingStake)} は差引済み · TRY投入 ${yen(m.tryCommitted)}</p>
 ...
 <div class="vh-bottom">
  ...
  <div class="vh-actions"><div class="vh-title">今日の状況</div><div class="vh-kpis"><div><span>精算</span><b>${m.settledCount}R</b></div><div><span>的中</span><b>${m.hits}</b></div><div><span>的中率</span><b>${pct(m.hitRate)}</b></div><div><span>未精算</span><b>${m.pendingCount}R</b></div></div><div class="vh-action-grid"><button type="button" data-vh-jump="predict">12Rを見る</button><button type="button" data-vh-jump="results">結果を見る</button></div><div class="vh-note">1点 ${yen(PICK_PRICE)} の仮想運用。予想と結果の境界は従来どおり維持します。</div></div>
 </div>`;
...
window.BOAT_COMMAND_VIRTUAL_HOME_V03515=Object.freeze({version:VERSION,render,realMoney:false});
