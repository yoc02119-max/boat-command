...
async function answer(text){const q=String(text||'').normalize('NFKC').trim();if(!q)return'';const llm=window.BOAT_JARVIS_LLM_CLIENT_V0336;if(llm?.chat){try{return await llm.chat(q,snapshot())}catch(e){console.warn('JARVIS LLM fallback',e)}}const agent=app();if(!agent?.execute)return'JARVISの会話バックエンドが未接続です。';return await agent.execute(q)}
